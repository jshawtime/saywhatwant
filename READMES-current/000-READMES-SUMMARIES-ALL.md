...
**Total Documents:** 150 READMEs + 2 documentation directories
...
**Tags:** #queue-monitor #bugfix #crash-fix #llm-requests #error-handling #websocket  
**Summary Created:** October 22, 2025

Documents critical fixes for two major Queue Monitor issues: dashboard crashing (black screen) when messages arrive and LLM requests not displaying. Issue 1 root cause was unsafe access to `item.entity.modelServer` without null checks causing React rendering errors. Fixed with safe property access pattern checking object existence before accessing nested properties, plus fallbacks for priority and message fields. Issue 2 root cause was LLM request objects lacking unique identifiers (id, timestamp), causing React key issues and rendering problems. Fixed by adding `id: llm-${Date.now()}-${entity.id}` and `timestamp: Date.now()` to llmRequest object in bot code, plus dashboard fallbacks for field variations (modelName/model, entityId/entity). Implemented error boundary pattern with renderError state, try-catch wrapper, and user-friendly error screen with reload button. Added debug logging for queue and LLM request updates via useEffect hooks. Includes complete deployment steps (rebuild bot, restart PM2), testing procedures (queue crash test, LLM display test, error recovery test), debugging guide (PM2 logs, browser console, WebSocket verification), and detailed message flow diagrams for both LLM requests and queue items. Files modified: AI-Bot-Deploy/src/index.ts (added tracking IDs), Queue-Monitor-Deploy/src/App.tsx (safe rendering, error boundary, debug logs). Success criteria includes no crashes on new messages, proper LLM request display with all metadata, working copy/expand functions, and error recovery.
