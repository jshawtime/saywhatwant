...
export function rgbToNineDigit(color: string): string {
  // Already in 9-digit format (with optional suffix)
...
export function nineDigitToRgb(digits: string): string {
  ...
  const cleanDigits = digits.split('-')[0];

  if (!/^\d{9}$/.test(cleanDigits)) return 'rgb(96, 165, 250)';
  
  const r = parseInt(cleanDigits.slice(0, 3), 10);
  const g = parseInt(cleanDigits.slice(3, 6), 10);
  const b = parseInt(cleanDigits.slice(6, 9), 10);
